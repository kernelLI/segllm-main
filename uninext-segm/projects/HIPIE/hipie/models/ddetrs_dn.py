...
class DDETRSegmUniDN(nn.Module):
    ...
    def convert_dtype(self,data, dtype):
        if isinstance(data, torch.Tensor):
            ...
            return {key: self.convert_dtype(value, dtype) for key, value in data.items()}
        elif isinstance(data, list):
            return [self.convert_dtype(item, dtype) for item in data]
        elif isinstance(data, tuple):
            return tuple(self.convert_dtype(item, dtype) for item in data)
        else:
            return data
        
    def coco_forward(self, samples, gt_targets, criterion, train=False, language_dict_features=None, task=None,features=None, pos=None,image_sizes=None):
        ...
        if features is None:
            ...
            assert features is not None and pos is not None and image_sizes is not None
        ...
        out_fg = self.convert_dtype(out_fg,torch.float)
        out_bg = self.convert_dtype(out_bg,torch.float)
        out_gt = self.convert_dtype(out_gt,torch.float)
    ...
    def coco_inference(self, samples, gt_targets, criterion, train=False, language_dict_features=None, 
                       task=None,
                       bg_queries_lang=None,
                       features=None, pos=None,image_sizes=None,
                       ):
        if features is None:
            image_sizes = samples.image_sizes
            if not isinstance(samples, NestedTensor):
