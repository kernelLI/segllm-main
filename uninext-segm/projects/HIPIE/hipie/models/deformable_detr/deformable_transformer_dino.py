...
def agg_lang_feat(features, mask, pool_type="average"):
    ...
    if pool_type == "average":
        embedded = features * mask.unsqueeze(-1).to(features.dtype) # use mask to zero out invalid token features
        aggregate = embedded.sum(1) / (mask.sum(-1).unsqueeze(-1).to(features.dtype))
    elif pool_type == "max":
        out = []
...
class DeformableTransformerVLDINO(nn.Module):
    ...
    attn_masks=None, return_src_info=False,bg_queries_lang=None,enc_mask=False):
        ...
        valid_ratios.to(src_flatten)
        # encoder & VL early fusion
        with torch.autocast(device_type='cuda', enabled=False, dtype=torch.float):
            self.encoder.to(torch.float)
        ...
        lang_feat_pool = lang_feat_pool.to(torch.bfloat16)
        # self.resizer.to(torch.bfloat16)
        ...
        output_memory, output_proposals = self.gen_encoder_output_proposals(memory.to(torch.bfloat16), mask_flatten, spatial_shapes)
        ...
        if bg_queries_lang is not None and len(bg_queries_lang) > 0:
            ...
            reference_points = torch.cat([torch.zeros((bs,bg_queries_lang.shape[1],4)).to(srcs[0]).to(tgt.device), reference_points], dim=1)
        ...
        self.decoder.to(torch.float)
        hs, inter_references = self.decoder(tgt_new.to(torch.float), reference_points, memory,
