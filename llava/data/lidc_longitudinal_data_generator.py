    ...
    def _create_longitudinal_pair(self, patient_id: str, baseline: NoduleInfo, followup: NoduleInfo) -> Optional[LongitudinalPair]:
        ...
        for b_nodule in baseline_nodules:
            for f_nodule in followup_nodules:
                distance = self._calculate_centroid_distance(b_nodule.centroid, f_nodule.centroid)
                if distance < 30.0:  # 30mm阈值
                    nodule_id = b_nodule.nodule_id
                    matched_nodules[nodule_id] = (b_nodule, f_nodule)
                    
                    # 计算变化指标
                    changes[nodule_id] = self._calculate_nodule_changes(b_nodule, f_nodule)
