...
class MinimalSiameseUNet(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # 解码器路径
        dec3 = self.dec3(F.interpolate(diff_features, scale_factor=2, mode='trilinear', align_corners=False))
        dec2 = self.dec2(F.interpolate(dec3, scale_factor=2, mode='trilinear', align_corners=False))
        ...
        for batch_data in dataloader:
            ...
            for sample_data in batch_data:
                ...
                if pred_mask.shape != mask_t2.shape:
                    # 调整预测尺寸以匹配目标
                    pred_mask = F.interpolate(pred_mask, size=mask_t2.shape[2:], mode='trilinear', align_corners=False)
